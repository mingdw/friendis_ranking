...
func initRedisClientPool() *redis.Pool {
	redisPool = &redis.Pool{
		...
		IdleTimeout: variable.YamlConfig.GetDuration("Redis.IdleTimeout") * time.Second, //最大的空闲连接等待时间，超过此时间后，空闲连接将被关闭
		Dial: func() (redis.Conn, error) {
			//此处对应redis ip及端口号
			conn, err := redis.Dial("tcp", variable.YamlConfig.GetString("Redis.Host")+":"+variable.YamlConfig.GetString("Redis.Port"))
			if err != nil {
				variable.ZapLog.Error(errorMsg.ErrorsRedisInitConnFail + err.Error())
				return nil, err
			}
			auth := variable.YamlConfig.GetString("Redis.Auth") //通过配置项设置redis密码
			if len(auth) >= 1 {
				if _, err := conn.Do("AUTH", auth); err != nil {
					_ = conn.Close()
					variable.ZapLog.Error(errorMsg.ErrorsRedisAuthFail + err.Error())
				}
			}
			_, _ = conn.Do("select", variable.YamlConfig.GetInt("Redis.IndexDb"))
			return conn, err
		},
	}
...
func GetOneRedisClient() *RedisClient {
	...
	for i := 1; i <= maxRetryTimes; i++ {
		...
		if err := oneConn.Err(); err != nil {
			...
			if i == maxRetryTimes {
				variable.ZapLog.Error(errorMsg.ErrorsRedisGetConnFail, zap.Error(oneConn.Err()))
				return nil
			}
			//如果出现网络短暂的抖动，短暂休眠后，支持自动重连
			time.Sleep(time.Second * variable.YamlConfig.GetDuration("Redis.ReConnectInterval"))
		} else {
			break
		}
	}
