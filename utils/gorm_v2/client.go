...
func GetOneMysqlClient() (*gorm.DB, error) {
	sqlType := "Mysql"
	readDbIsOpen := variable.YamlConfig.GetInt("Gormv2." + sqlType + ".IsOpenReadDb")
...
func GetSqlDriver(sqlType string, readDbIsOpen int, dbConf ...ConfigParams) (*gorm.DB, error) {
	...
	if val, err := getDbDialector(sqlType, "Write", dbConf...); err != nil {
		variable.ZapLog.Error(errorMsg.ErrorsDialectorDbInitFail+sqlType, zap.Error(err))
	} else {
		dbDialector = val
	}
	...
	if readDbIsOpen == 1 {
		...
		err = gormDb.Use(dbresolver.Register(resolverConf).SetConnMaxIdleTime(time.Second * 30).
			SetConnMaxLifetime(variable.YamlConfig.GetDuration("Gormv2."+sqlType+".Read.SetConnMaxLifetime") * time.Second).
			SetMaxIdleConns(variable.YamlConfig.GetInt("Gormv2." + sqlType + ".Read.SetMaxIdleConns")).
			SetMaxOpenConns(variable.YamlConfig.GetInt("Gormv2." + sqlType + ".Read.SetMaxOpenConns")))
		if err != nil {
			return nil, err
		}
	}
	...
	if rawDb, err := gormDb.DB(); err != nil {
		...
		rawDb.SetConnMaxLifetime(variable.YamlConfig.GetDuration("Gormv2."+sqlType+".Write.SetConnMaxLifetime") * time.Second)
		rawDb.SetMaxIdleConns(variable.YamlConfig.GetInt("Gormv2." + sqlType + ".Write.SetMaxIdleConns"))
		rawDb.SetMaxOpenConns(variable.YamlConfig.GetInt("Gormv2." + sqlType + ".Write.SetMaxOpenConns"))
		// 全局sql的debug配置
		if variable.YamlConfig.GetBool("Gormv2.SqlDebug") {
...
func getDbDialector(sqlType, readWrite string, dbConf ...ConfigParams) (gorm.Dialector, error) {
	...
	default:
		return nil, errors.New(errorMsg.ErrorsDbDriverNotExists + sqlType)
	}
...
func getDsn(sqlType, readWrite string, dbConf ...ConfigParams) string {
	Host := variable.YamlConfig.GetString("Gormv2." + sqlType + "." + readWrite + ".Host")
	DataBase := variable.YamlConfig.GetString("Gormv2." + sqlType + "." + readWrite + ".DataBase")
	Port := variable.YamlConfig.GetInt("Gormv2." + sqlType + "." + readWrite + ".Port")
	User := variable.YamlConfig.GetString("Gormv2." + sqlType + "." + readWrite + ".User")
	Pass := variable.YamlConfig.GetString("Gormv2." + sqlType + "." + readWrite + ".Pass")
	Charset := variable.YamlConfig.GetString("Gormv2." + sqlType + "." + readWrite + ".Charset")
	...
	switch strings.ToLower(sqlType) {
	case "mysql":
		return fmt.Sprintf("%s:%s@tcp(%s:%d)/%s?charset=%s&parseTime=false&loc=Local&parseTime=true", User, Pass, Host, Port, DataBase, Charset)
