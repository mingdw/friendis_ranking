...
func CreateZapFactory(entry func(zapcore.Entry) error) *zap.Logger {
	...
	appDebug := variable.YamlConfig.GetBool("AppDebug")
	...
	timePrecision := variable.YamlConfig.GetString("Logs.TimePrecision")
	...
	switch variable.YamlConfig.GetString("Logs.TextFormat") {
	case "console":
	...
	//写入器
	fileName := variable.BasePath + variable.YamlConfig.GetString("Logs.GoSkeletonLogName")
	lumberJackLogger := &lumberjack.Logger{
		Filename:   fileName,                                      //日志文件的位置
		MaxSize:    variable.YamlConfig.GetInt("Logs.MaxSize"),    //在进行切割之前，日志文件的最大大小（以MB为单位）
		MaxBackups: variable.YamlConfig.GetInt("Logs.MaxBackups"), //保留旧文件的最大个数
		MaxAge:     variable.YamlConfig.GetInt("Logs.MaxAge"),     //保留旧文件的最大天数
		Compress:   variable.YamlConfig.GetBool("Logs.Compress"),  //是否压缩/归档旧文件
	}
