...
func CreateUsersTokenCacheFactory(userId string) *userTokenCacheRedis {
	redCli := redis_factory.GetOneRedisClient()
	if redCli == nil {
		return nil
	}
	return &userTokenCacheRedis{redisClient: redCli, userTokenKey: "token_userid_" + userId}
}
...
func (u *userTokenCacheRedis) DelOverMaxOnlineCache() bool {
	...
	onlineUsers := variable.YamlConfig.GetInt("Token.JwtTokenOnlineUsers")
