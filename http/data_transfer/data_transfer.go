	...
	if tmpBytes, err1 := json.Marshal(validatorInterface); err1 == nil {
		if err2 := json.Unmarshal(tmpBytes, &tempJson); err2 == nil {
			if value, ok := tempJson.(map[string]interface{}); ok {
				...
				curDateTime := time.Now().Format(globalConst.DateFormat)
				...
				context.Set(extraAddDataPrefix+"creator", globalConst.SysAccount)
				context.Set(extraAddDataPrefix+"editor", globalConst.SysAccount)
				return context
			}
		}
	}
